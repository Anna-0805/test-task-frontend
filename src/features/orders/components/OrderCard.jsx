...
const OrderCard = ({ 
  ...
  const isActive = isGroupsPage && selectedOrderId === order.id; 
  ...
  return (
    <div 
      ...
      onClick={() => isGroupsPage && dispatch(setSelectedOrderId(order.id))} 
    > 
      {!isCompressed && ( 
      ...
      <div className="orders-page__card-menu d-flex align-items-center gap-3"> 
        <button className="orders-page__menu-btn btn d-flex align-items-center justify-content-center p-0 border border-secondary-subtle rounded-circle"> ☰ </button> 
        <div> 
          <div className="fw-bold text-dark h5 m-0">{count}</div> 
          <div className="orders-page__menu-subtext text-muted small text-lowercase"> продукта </div> 
        </div> 
      </div> 
      ...
      {!isCompressed && ( 
        <div className="orders-page__card-price text-start"> 
          <div className="orders-page__price-usd text-muted small">{totalUSD} $</div> 
          <div className="orders-page__price-uah fw-bold text-dark">{totalUAH} UAH</div> 
        </div> 
      )} 
      ...
      {isCompressed && isActive && ( 
        <div className="orders-page__card-arrow d-flex align-items-center justify-content-center"> ❯ </div> 
      )} 
    </div>
  );
};
