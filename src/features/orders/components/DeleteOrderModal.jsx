const DeleteOrderModal = ({ isOpen, onClose, onConfirm, orderTitle, products = [] }) => {
  if (!isOpen) return null;

  return (
    <div className="orders-popup modal fade show d-block" tabIndex="-1" role="dialog">
        ...
        <div className="modal-content border-0 shadow position-relative bg-white orders-popup__content">
          ...
          <div className="modal-body p-4 bg-white orders-popup__body">
            ...
            <h5 className="orders-popup__title fw-bold text-dark mb-4">
              Вы уверены, что хотите удалить {orderTitle || 'этот приход'}?
            </h5>
          ...
          <div className="orders-popup__footer d-flex justify-content-end align-items-center p-3 gap-3">
            <button 
              className="orders-popup__cancel-btn btn btn-link text-white text-uppercase fw-bold text-decoration-none small p-0" 
              onClick={onClose}
            >
              Отменить
            </button>
